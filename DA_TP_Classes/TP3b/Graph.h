...
public:
    ...
    std::vector<T> getPath(const T &origin, const T &dest); //TODO...
    ...
    for (int i = 1; i < vertexSet.size(); i++) {
        for (auto vertex : vertexSet) {
            for (Edge<T> e : vertex->adj) {
                if (e.dest->dist > vertex->dist + e.weight) {
                    e.dest->dist = vertex->dist + e.weight;
                    e.dest->path = vertex;
                }
            }
        }
...
std::vector<T> Graph<T>::getPath(const T &origin, const T &dest) {
    std::vector<T> res;
    bellmanFordShortestPath(origin);

    if (findVertex(dest)->dist == INF) return res;

    res.push_back(dest);
    T temp = dest;
    while (temp != origin) {
        temp = findVertex(temp)->path->info;
